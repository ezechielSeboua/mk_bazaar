...
class OrderController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->filled('status')) {
            // ✅ trim() + strtolower() : MySQL insensible à la casse, PostgreSQL non
            $query->where('status', strtolower(trim($request->status)));
        }

        $orders = $query->latest()->get();

        // ✅ (int)(float) : PostgreSQL retourne les sum() en string, MySQL en int
        $totalRevenue     = (int) (float) Order::where('status', 'completed')->sum('total_price');
        $pendingCount     = Order::where('status', 'pending')->count();
        $cancelledRevenue = (int) (float) Order::where('status', 'cancelled')->sum('total_price');
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            $order = DB::transaction(function () use ($validated) {
                ...
                $order = Order::create([
                    'user_id'           => auth('api')->id(),
                    // ✅ Compatible MySQL et PostgreSQL : pas de collision sous charge
                    'order_number'      => 'MK-' . strtoupper(Str::random(4)) . '-' . now()->format('YmdHis') . '-' . strtoupper(Str::random(3)),
                    'delivery_location' => trim($validated['delivery_location']),
                    'detailed_address'  => trim($validated['detailed_address']),
                    'delivery_fee'      => $validated['delivery_fee'],
                    'total_price'       => $validated['total_price'],
                    'status'            => 'pending',
                ]);
                ...
                foreach ($validated['items'] as $itemData) {
                    // ✅ lockForUpdate() fonctionne sur MySQL (InnoDB) et PostgreSQL
                    ...
                    // ✅ Cast universel : évite les strings de PostgreSQL et les floats de MySQL
                    $price = (int) (float) ($variant->price ?? $variant->product->price);
            ...
            // ✅ fresh() : relit depuis la BD après transaction (utile surtout pour PostgreSQL)
            return response()->json([
                'success' => true,
                'message' => 'Commande enregistrée avec succès !',
                'order'   => $order->fresh()->load('items.variant.product')
            ], 201);
        ...
        try {
            DB::transaction(function () use ($order, $validated) {
                ...
                if ($validated['status'] === 'cancelled' && $order->status !== 'cancelled') {
                    ...
                    foreach ($order->items as $item) {
                        // ✅ lockForUpdate()->first() : évite les valeurs cachées sur les deux SGBD
            ...
            // ✅ fresh() : données à jour après transaction sur MySQL et PostgreSQL
            return response()->json([
                'success' => true,
                'message' => 'Statut de la commande mis à jour.',
                'order'   => $order->fresh()->load('items.variant.product')
