// components/CharacterLoader.tsx
import { useEffect, useRef } from 'react';
import Emoji from './Emoji'; // Assure-toi que le chemin est correct

interface CharacterLoaderProps {
  message?: string;
  emojiType?: 'smile' | 'wink' | 'surprised' | 'tongue' | 'star' | 'heart' | 'crying_smile' | 'crying' | 'unamused' | 'face_in_clouds' | 'slight_smile' | 'persevere' | 'thinking' | 'cool'; // Ajoute les types d'emojis que tu souhaites
}
...
export default function CharacterLoader({
  ...
  useEffect(() => {
    if (!document.getElementById('character-loader-styles')) {
      const style = document.createElement('style');
      style.id = 'character-loader-styles';
      style.textContent = `
        .character-loader-wrapper {
          display: flex;
        ...
        @keyframes lstep {
          ...
          33%  { transform: rotate(35deg) translateY(-15px) translateX(32px); }
          66%  { transform: rotate(-25deg) translateY(0) translateX(25px); }
          100% { transform: rotate(-5deg); }
        }
        @keyframes rstep {
          0%   { transform: rotate(-5deg); }
          33%  { transform: rotate(35deg) translateY(-10px) translateX(30px); }
          66%  { transform: rotate(-25deg) translateY(0) translateX(20px); }
          100% { transform: rotate(-5deg); }
        }
        @keyframes scroll {
          0%   { transform: translateY(25px) translateX(50px) rotate(10deg); opacity: 0; }
          33%  { opacity: 0.25; }
          66%  { opacity: 0.25; }
          100% { transform: translateY(-50px) translateX(-100px) rotate(10deg); opacity: 0; }
        }
        .character-loader-message {
          margin-top: 20px;
          font-size: 0.875rem;
    ...
    return () => {
      const style = document.getElementById('character-loader-styles');
      if (style) {
        style.remove();
      }
    };
  ...
  return (
    <div className="character-loader-wrapper">
      <div className="loader-svg-container">
        ...
        >
          <g transform="translate(-201.44063,-235.75466)" strokeMiterlimit="10">
        ...
        >
          <g transform="translate(-241.54137,-218.44347)" strokeMiterlimit="10">
            <path
        ...
        >
          <g transform="translate(-164.41679,-112.94712)" strokeMiterlimit="10">
            <path
        ...
        >
          <g transform="translate(-191.87889,-75.62023)" strokeMiterlimit="10">
            <path d="" fill="#ffffff" stroke="none" strokeWidth="0.5" />
            <path
        ...
        >
          <g transform="translate(-176.25,-207.64957)" stroke="#000000" strokeWidth="2.5" strokeLinecap="round" strokeMiterlimit="10">
      ...
      {message && <p className="character-loader-message">{message}</p>}
