...
const CategoryIcon = () => (
  <svg
    ...
    strokeWidth={1.5}
    stroke="currentColor"
    className="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-1 text-stone-500"
  >
    <path
      strokeLinecap="round"
...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-xl border border-stone-200 h-[300px] sm:h-[400px] animate-pulse" />
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-4 space-y-1">
        ...
        <div className="mt-auto pt-2 text-sm sm:text-base font-black text-stone-950">
    ...
    <section className="px-4 py-8 sm:py-12 bg-[#FAFAFA]">
      <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {loading
          ...
          : products.map((p, i) => (
              <GridProductCard key={p.id || i} product={p} index={i} />
            ))}
      </div>
    </section>
  );
