...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-xl border border-stone-200 overflow-hidden h-[240px] sm:h-[420px] md:h-[460px] animate-pulse flex flex-col justify-between">
      <div className="w-full h-[48%] bg-stone-200" />
      <div className="p-1.5 flex-grow flex flex-col justify-between">
        <div className="space-y-1.5">
          <div className="flex justify-between">
            <div className="h-2.5 bg-stone-200 rounded w-1/3" />
            <div className="h-2.5 bg-stone-200 rounded w-8" />
          </div>
          <div className="h-3.5 bg-stone-200 rounded w-3/4 mt-0.5" />
          <div className="h-2.5 bg-stone-200 rounded w-5/6" />
        </div>
        <div className="mt-auto space-y-1 pt-0.5 border-t border-stone-100">
          ...
          inStockProducts.map((product, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -6 }}
                className="group flex flex-col justify-between bg-white rounded-xl border border-stone-200/80 overflow-hidden h-[240px] sm:h-[380px] md:h-[460px] transition-all duration-300 hover:border-stone-400 hover:shadow-md"
              >
                <Link
                ...
                {/* <div className="p-1 sm:p-3 bg-white border-t border-stone-100">
                                    <Link to={`/products/${product.slug}`} className="block">
                                        <motion.div
                                            ...
                                            whileTap={{ scale: 0.98 }}
                                            className="w-full py-0.5 sm:py-2.5 rounded-lg flex items-center justify-center gap-0.5 text-[8px] sm:text-xs font-bold uppercase tracking-wider bg-stone-950 text-white hover:bg-green-600 shadow-sm transition-all duration-200 min-h-[44px]"
                                        >
                                            <WhatsAppIcon />
                                            <span>Commander</span>
                                        </motion.div>
                ...
                <div className="p-1 sm:p-3 bg-white border-t border-stone-100 flex justify-center">
                  ...
                  >
                    <motion.div
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      className="px-4 py-2 sm:w-full rounded-lg flex items-center justify-center gap-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-wider bg-stone-950 text-white hover:bg-green-600 shadow-sm transition-all duration-200"
                    >
                      <WhatsAppIcon />
                      {/* Texte adaptatif */}
