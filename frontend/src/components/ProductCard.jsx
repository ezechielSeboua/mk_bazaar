...
export default function ProductCard({ product, delay = 0 }) {
  ...
  return (
    ...
    >
      <Link to={`/products/${product.slug}`} className="block aspect-[4/5] sm:aspect-[3/4] bg-stone-100 overflow-hidden">
        <img src={backgroundImage} alt={name} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out" />
        {hasDiscount && (
        ...
        {!isAvailable && (
          <div className="absolute inset-0 bg-white/80 flex items-center justify-center">
            <span className="text-[10px] font-bold uppercase text-stone-600 tracking-widest">Épuisé</span>
          </div>
        )}
      </Link>
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-4 space-y-2">
        ...
        <div className="mt-auto pt-2 border-t border-stone-100 flex items-end justify-between">
          <div className="flex flex-col">
            <span className="text-base sm:text-lg font-black text-stone-950">{currentPrice.toLocaleString()} FCFA</span>
            {oldPrice && oldPrice > currentPrice && <span className="text-[11px] text-stone-400 line-through">{oldPrice.toLocaleString()}</span>}
          </div>
      ...
      {showVariantModal && (
        <VariantPickerModal product={product} variants={variants} onSelect={addToCart} onClose={() => setShowVariantModal(false)} />
      )}
    </motion.div>
  );
}