...
export default function DashboardLayout({ children }) {
  ...
  const menuItems = [
    ...
    { path: '/dashboard/categories', label: 'Catégories', icon: <FolderIcon /> },
    { path: '/dashboard/products', label: 'Produits', icon: <PackageIcon /> },
    { path: '/dashboard/commands', label: 'Commandes', icon: <ClipboardIcon /> },
    { path: '/dashboard/users', label: 'Utilisateurs', icon: <UsersIcon /> },
    { path: '/dashboard/finances', label: 'Finances', icon: <FinanceIcon /> },
    { path: '/dashboard/configurations', label: 'Configurations', icon: <SettingsIcon /> },
  ];
  ...
  return (
    <div className="flex h-screen bg-[#F9F9F7]">
      {/* Sidebar Desktop */}
      <div className={`hidden md:flex ${sidebarOpen ? 'w-64' : 'w-20'} bg-black text-[#F9F9F7] transition-all duration-300 flex-col`}>
        <div className="p-6 flex items-center justify-between border-b border-stone-700">
          {sidebarOpen && (
            <h1 className="text-sm font-bold uppercase tracking-widest">MK Admin</h1>
          )}
          <button
