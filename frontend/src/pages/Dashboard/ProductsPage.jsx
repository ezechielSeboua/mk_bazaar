...
const EMPTY_FORM = {
  ...
  price: "", category_id: "", in_stock: true, featured: false,
};
...
function SkeletonRow() {
  // Correction Tailwind : Classes écrites en entier pour éviter le bypass du compilateur
  return (
    <motion.tr
    ...
    >
      {["w-10", "w-32", "w-20", "w-16", "w-16", "w-12", "w-12"].map((wClass, i) => (
...
export default function ProductsPage() {
  ...
  // Garder une référence synchronisée des previews pour le nettoyage final sans closure bug
  ...
  /* ── Nettoyage : révoquer les object URLs à l'unmount ── */
  useEffect(() => {
    return () => {
  ...
  const resetFilters = useCallback(() => {
    ...
    setStockFilter(""); setFeaturedFilter("");
  }, []);
  ...
  const handleFilesChange = useCallback((e) => {
    ...
    // Unification de la mise à jour pour éviter les batchings redondants
    setPreviewUrls((prev) => {
  ...
  const validateForm = useCallback(() => {
    ...
    // Une image requise seulement si création et aucune image existante
  ...
  const resetForm = useCallback(() => {
    ...
    setSelectedFiles([]); setExistingImages([]);
  }, []);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      // Correction : Envoi des images conservées lors d'un Edit pour synchronisation backend
      if (editingId) {
        existingImages.forEach((img) => fd.append("existing_images[]", img.url));
      }
  ...
  }, [formData, editingId, selectedFiles, existingImages, validateForm, resetForm, setProducts, showSuccess]);
  ...
  const handleEdit = useCallback((product) => {
    setFormData({
      ...
      in_stock: product.in_stock ?? true, featured: product.featured ?? false,
    });
    setExistingImages(
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        >
          <div>
            <h1 className="text-3xl font-light uppercase tracking-tight">Produits</h1>
            <p className="text-stone-600 text-sm mt-1">
              {isLoading && products.length === 0
                ? <span className="inline-block w-20 h-4 bg-stone-200 rounded animate-pulse" />
                : <motion.span key={`${filteredProducts.length}-${products.length}`}
        ...
        {!showForm && (
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-end gap-4">
              ...
              <FilterSelect label="Catégorie" icon={<Tag className="w-4 h-4 text-stone-400" />}
              ...
              <FilterSelect label="Stock" icon={<Package className="w-4 h-4 text-stone-400" />}
              ...
              <FilterSelect label="Mis en avant" icon={<Star className="w-4 h-4 text-stone-400" />}
            ...
            >
              <div className="bg-white border border-stone-200 rounded-lg p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 max-w-3xl">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Field label="Prix (FCFA) *" error={errors.price}>
                      <input type="number" name="price" value={formData.price}
                        onChange={handleChange} min="0" className={inputCls(errors.price)} />
                    </Field>
                  </div>
                  ...
                  {/* Upload */}
                  <Field label={editingId ? "Ajouter des images" : "Images *"} error={errors.images}>
                    ...
                    {previewUrls.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-2">
                        {previewUrls.map((url, i) => (
                          <img key={i} src={url} alt="" className="w-20 h-20 object-cover rounded border border-stone-200" />
                        ))}
                      </div>
                    )}
                  </Field>
        ...
        {/* Tableau */}
        <motion.div className="bg-white border border-stone-200 rounded-lg"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  {["Image","Nom","Catégorie","Prix","Stock","Featured","Actions"].map((h) => (
                    <th key={h} className="text-left py-3 px-6 font-bold uppercase text-[10px] tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {isLoading && products.length === 0 ? (
                  ...
                  <EmptyRow colSpan={7} message="Aucun produit pour le moment." />
                ) : filteredProducts.length === 0 ? (
                  <EmptyRow colSpan={7} message="Aucun produit ne correspond aux filtres.">
                    {hasActiveFilters && (
                      <button type="button" onClick={resetFilters}
                  ...
                  filteredProducts.map((product, index) => (
                    <motion.tr key={product.id}
                      ...
                      custom={index} variants={tableRow} initial="hidden" animate="visible"
                    >
                      <td className="py-4 px-6">
                        <div className="w-10 h-10 rounded bg-stone-100 border border-stone-200 overflow-hidden flex items-center justify-center">
                          {product.image_path?.[0]
                            ? <img src={resolveMediaUrl(product.image_path[0])} alt={product.name} className="w-full h-full object-cover" />
                      ...
                      <td className="py-4 px-6">
                        <Badge on={product.in_stock} onLabel="En stock" offLabel="Rupture"
                          onCls="bg-green-100 text-green-800" offCls="bg-red-100 text-red-800" />
                      </td>
                      <td className="py-4 px-6">
                        <Badge on={product.featured} onLabel="Oui" offLabel="Non"
                          onCls="bg-blue-100 text-blue-800" offCls="bg-stone-100 text-stone-500" />
                      </td>
                      <td className="py-4 px-6 relative overflow-visible" ref={openMenuId === product.id ? menuContainerRef : null}>
                        <button onClick={() => setOpenMenuId((id) => id === product.id ? null : product.id)}
        ...
        <ConfirmDialog
          ...
          message={<>Voulez-vous vraiment supprimer <span className="font-semibold text-black">"{confirmDelete.name}"</span> ? Cette action est irréversible.</>}
...
function Field({ label, error, children }) {
  return (
    <div>
      <label className="text-[10px] uppercase tracking-wider font-bold block mb-2">{label}</label>
      {children}
...
// Petite optimisation : Utilisation de template literals natifs au lieu de mini conditions complexes
function Badge({ on, onLabel, offLabel, onCls, offCls }) {
  return (
    <span className={`px-3 py-1 rounded text-[10px] font-bold ${on ? onCls : offCls}`}>
      {on ? onLabel : offLabel}
    </span>
  );
}
...
function EmptyRow({ colSpan, message, children }) {
  return (
    <tr>
      <td colSpan={colSpan} className="py-12 text-center text-stone-400 text-sm">
        {message}{children}
      </td>
    </tr>
  );
